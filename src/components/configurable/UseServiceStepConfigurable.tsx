  ...
  async function handleRun(action: ServiceAction) {
    ...
    try {
      ...
      onApiCall?.({
        ...
        method: 'POST',
        endpoint:
          'POST /api/configurable/redeem → Coinflow /redeem/evm/creditsAuthMsg',
        payload: {...redeemPayloadBase, action: 'auth'},
        status: 0,
        response: 'pending…',
      });

      const authResponse = await axios.post('/api/configurable/redeem', {
        action: 'auth',
        ...redeemPayloadBase,
      });
      ...
      onApiCall?.({
        ...
        method: 'POST',
        endpoint:
          'POST /api/configurable/redeem → Coinflow /redeem/evm/sendGaslessTx',
        payload: {
          ...redeemPayloadBase,
          action: 'execute',
      ...
      const executeResponse = await axios.post('/api/configurable/redeem', {
        action: 'execute',
        ...redeemPayloadBase,
        signedMessage,
        validBefore,
      ...
      if (axios.isAxiosError(err)) {
        ...
        onApiCall?.({
          ...
          method: 'POST',
          endpoint:
            runningId?.includes('execute') ??
            false
              ? 'POST /api/configurable/redeem → Coinflow /redeem/evm/sendGaslessTx'
              : 'POST /api/configurable/redeem → Coinflow /redeem/evm/creditsAuthMsg',
          payload: {...redeemPayloadBase},
          status,
